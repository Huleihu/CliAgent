...
- S13 下一步在 CLI 组合根创建共享的后台任务仓储、标识生成器、命令执行器、线程服务和通知源，注册父侧 `BashTool` 并增加条件式后台执行提示；S6 子 Agent 白名单不继承该工具。完成后运行完整 pytest 与 `python -m ruff check src tests`，并更新 README 的 CLI 能力说明。
