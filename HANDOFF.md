...
- `python -m pytest`：470 passed（覆盖既有 S1-S9 闭环，以及 S10 系统提示 section 组装、缓存和 CLI 条件加载）。
...
- 继续 S10 第 3 步：为 `MinimalAgentLoop` 增加向后兼容的动态系统提示提供端口，在每次模型调用前取得最新基础提示并保留 Todo reminder 的一次性派生语义；暂不改变 S8/S9 的后续派生顺序。
